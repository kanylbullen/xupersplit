...
export function AccountMenu({
  ...
  return (
    // Uncontrolled on purpose: <details> owns its open state, we only mirror it
    // into React so the outside-click/Escape listeners attach when it's open.
    <details
      ref={ref}
    ...
    >
      ...
      <div className="absolute right-0 z-50 mt-2 w-72 rounded-2xl border border-stone-200/80 bg-surface p-1.5 shadow-lg">
        ...
        {displayName ? (
          <>
            {/* Renders nothing inside the Farcaster Mini App — the rows below
                just close up, no dangling divider. */}
            <RegisterPasskeyButton className={ROW} />
